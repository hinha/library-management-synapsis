...
type ICacheRepository interface {
	...
	GetUser(ctx context.Context, id uint) (user *User, err error)
}
...
func (r *CacheRepository) SaveUser(ctx context.Context, user *User) error {
	data, _ := json.Marshal(user)
	return r.client.Set(ctx, config.RedisKeyUserPrefix+fmt.Sprintf("%d", user.ID), data, config.JwtTokenExpiration).Err()
}

func (r *CacheRepository) GetUser(ctx context.Context, id uint) (user *User, err error) {
